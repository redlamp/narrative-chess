...
Beyond pure prototype. Includes: playable local chess, study replay, city-aware board, structured content editors, shared layout tooling, GitHub Pages deployment. Not database-backed — mix of browser-local state, optional local file saves, checked-in content.
...
Main problem: data-source ambiguity between browser state, local file state, repo-tracked files, deployed state.
...
Done: Edinburgh mapped, district metadata in panels, MapLibre + Google maps, Cities editor

Remaining:
- multiple playable cities wired into Play
- cleaner canonical data flow: content/editor → gameplay
- stronger published vs local-only board edit distinction
...
Groundwork: refined 2D presentation, map overlays, SVG piece assets, board + map motion
...
Done: local save/load flows, checked-in content, browser draft persistence, Pages deployment
...
**8.1 Data Canonicality** — Main issue. Browser drafts, local files, checked-in content, deployed state drift silently.
...
**8.4 UI Regression Risk High** — Layout mode, panel visibility, map hover/selection, story/panel sync, saved match state, content editor drafts = many failure surfaces.
...
Narrative Chess: serious local-first prototype. Real chess core, city-aware play, structured content editors, layout tooling, deployable static delivery.
