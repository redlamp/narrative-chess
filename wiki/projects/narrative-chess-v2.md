...
- **Library design pass shipped: 2026-05-11.** `/games` rebuilt as "The Catalogue" — editorial book-cover cards with outcome-coloured leather (navy active/open, forest won, oxblood lost, slate draw/aborted), a cursor-followed warm radial highlight driven globally by one window-level rAF tracker so every visible cover lights from the same cursor, and a floating cursor-pinned preview with three layered transform refs (instant follow / transitioned quadrant flip / perspective tilt) holding an `<AnimatedBoard>` that tweens pieces between FENs (greedy nearest-neighbour pairing per cohort, captured pieces drift to a strip on their own starting side). Tabs split Now playing / Archive; shelves stack active → your open → others' open. Open invitations show a colour-matched pawn beside the inviter's name. Archive cards decorate both names with a piece icon (king for winner, pawn for loser). Mobile renders an inline `<StaticBoard>` (112×112) on each card and disables hover effects via `@media (hover: hover)`. Dark mode page tokens lighten taupe (`#3d3830 / #2e2a23`) and layer a faint grain so the page reads as aged paper on the leather cover. Account page gets a Theme toggle so mobile users can switch themes (header hides toggle below 480px). Cross-cutting wins promoted to global memory: `gsap-clearprops-after-entrance.md`, `css-relative-oklch-and-property.md`, `media-hover-pointer-touch-safety.md`. Squash-merged via PR #46 (`06158b9` on `main`).
...
| Polish A + Polish C | (ship) | `697c963` | Polish A (draw-by-agreement): RPCs + GameActions banners + TerminalBanner subtitles. Polish C (mobile + desktop layout): board locked 576px, list snug 180px with absolute-inside-relative pattern, single-column move-list (chess.com convention), captured strip via container queries, header progressive-hide. Production deploy via PR #43. |
| Library design pass | (ship) | `06158b9` | `/games` rebuilt as "The Catalogue" — editorial book-cover cards (cover/page two-layer structure, outcome-coloured leather, cursor-followed warm radial highlight driven globally per frame), floating cursor-pinned preview with cross-FEN piece-tween AnimatedBoard + captured pile, tabs (Now playing / Archive), three-shelf Now-playing layout, archive piece icons, open-invite color pawn, mobile inline StaticBoard, dark-mode aged-paper grain, account page theme toggle. New: AnimatedBoard.tsx, CursorPreview.tsx, GameBook.tsx, GamesLibrary.tsx, StaticBoard.tsx, ColorPawn.tsx, hover-context.tsx. Production deploy via PR #46. |

## Staged on `dev` (post-library-pass, not yet on `main`)

(none, `dev` content-equal to `main` post-PR-#46 squash + reconciliation merge)
...
- `main` — production (latest squash: library design pass `06158b9`, 2026-05-11)
- `dev` — content-equal to `main` post-reconciliation merge `f629d96`
